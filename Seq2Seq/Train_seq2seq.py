...
class Training():
    ...
    def train(self, args, encoder,decoder, criterion, device, train_loader,test_loader,train_loss_loader,optimizer, epoch,
        ...
        for batch_idx, batch_ in enumerate(train_loader):
            ...
            inputs_en,inputs_de,target = self.batch_to_in_and_target(args,batch,before_lim,device)
            for opt in optimizer:
                opt.zero_grad()

            output_en,hidden_en1,hidden_en2 = encoder(inputs_en)
            output_ = decoder(inputs_de,hidden_en1,hidden_en2)


            # output_=output[:,-after_lim:,:]
            # output_ = torch.cat((output_en[:, -1, :].unsqueeze(dim=1), output_), dim=1)
            target_=target[:,-after_lim:,:]
            loss = criterion(output_, target_)#사이즈가 안맞은 output이 하나 작음 s2s는 그날꺼 넣어서 그날꺼 예측인가?
            loss.backward()
            for opt in optimizer:
                opt.step()
            ...
            if (steps % args.log_interval == 1) & (steps > args.log_pass):
                train_mse, train_mae, train_won_sum_mae, train_won_mae,train_mae_list,train_var = self.eval(args, train_loss_loader, encoder, decoder,
                                                                                   criterion, device,
                                                                                   normalize_factor,teacher_forcing=True)
                test_mse, test_mae, test_won_sum_mae, test_won_mae,test_mae_list,test_var= self.eval(args, test_loader, encoder, decoder, criterion,
                ...
                if is_best:
                    self.save_checkpoint(args, {
                        ...
                        'encoder_optimizer': optimizer[0].state_dict(),
                        'decoder_optimizer': optimizer[1].state_dict(),
                        'best_loss': best_loss,
                        'steps': steps,
    ...
    def batch_to_in_and_target(self,args,batch,before_lim,device):
        if args.isweather:
            ...
            try:
                ...
                pass
            #### 날씨하루 당기기#####
            edited[:,:-1:,5:] = edited[:,1:,5:]
        else:
            edited = batch[:, :, :5].clone().detach()
        ###############
        inputs_en = edited[:,:before_lim-1,:].clone().detach().to(device=device)
        inputs_de = torch.cat((edited[:,before_lim-1:-1,:2],edited[:,before_lim-1:-1,5:]),dim=2).clone().detach().to(device=device)
        target = edited[:,1:,2:5].clone().detach().to(device=device)

        return inputs_en,inputs_de,target

    def eval(self,args,data_loader,encoder, decoder,criterion,device,normalize_factor,teacher_forcing=False):
